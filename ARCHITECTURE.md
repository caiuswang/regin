...
**The allowlist is applied twice — at registration and at delivery.** `hook_manager/handlers/bridge_registry.py` reads `#{pane_current_command}` in the same tmux call that fetches the identity triple and writes **no row at all** for a pane whose foreground process isn't in `settings.agent_bridge.allowed_pane_commands`; the refusal is logged (`bridge_pane_register_refused`, naming the observed command) and deliberately not cached, so a transient mismatch heals on the next turn event. This is what stops a process regin *itself* spawned from claiming the server's own pane: such a child inherits `TMUX_PANE`, and its SessionStart hook would otherwise register the pane `regin serve` runs in, handing `/live` a steer composer that types into the operator's terminal. (`lib/agent_bridge/env.py`'s `child_env` closes the same hole from the other side, forcing `REGIN_BRIDGE=0` for every regin-launched child.) Registration-time refusal costs a session with an allowlisted-but-wrapped `claude` — one behind `caffeinate`/`script`, which reports the wrapper as its foreground command — its composer entirely, where before it got one that failed on send; widening `allowed_pane_commands` is the escape hatch.

**Storage:** `bridge_panes` (the session → pane registry, written by a SessionStart hook when the pane passes the allowlist) and `bridge_messages` (append-only inbox: sender, body, delivery outcome), both in `db/schema.sql`; `lib/agent_bridge/store.py` is the sole writer, mirroring `lib/agent_messages/store.py`.
...
**Cancelling a turn is not stopping the session, and needs its own latch.** `runner.interrupt()` ends the turn in flight and leaves everything else standing — the child, the channel, and the queue, because the operator's objection was to *this* turn and not to the three messages behind it. It raises `_interrupting` before the control message goes out, and `_drain_turn` races the turn against both that latch and `_stopping`: a CLI that honours an interrupt *silently* sends no `ResultMessage`, so without a latch of its own the pump would wait on `await done` forever and leave a session alive on paper that never runs another turn. The two arms differ only in what they leave behind — `_stopping` ends the pump, `_interrupting` returns to it. The latch is cleared at the *top* of each turn rather than the bottom: an interrupt asked for while nothing was running has nothing to cancel, and a latch left raised would abandon the next turn before it had said anything.

**A queued prompt can be corrected before it runs.** `_pending` holds `_Queued(id, text, cancelled)` records rather than bare strings, because `/live` has to name *one* entry to edit or drop and two identical prompts are two entries — position is not identity across the poll that renders a row and the request that acts on it. `edit_pending`/`cancel_pending` answer in-thread under a `threading.Lock` (a list walk and a field write; no reason to hop onto the loop and wait on a future for an answer the operator is holding the UI open for), and the pump takes a record off `_pending` under that same lock, which is what makes the cutoff exact: an edit either lands before the CLI is given the text or finds the record gone and refuses. Cancelling sets a flag instead of removing, since the record is by then inside the `asyncio.Queue`, which cannot be reached into — the pump skips it on the way past. This is a capability the tmux tier structurally lacks: that queue lives inside Claude Code, regin only replays its `queue-operation` entries from the transcript, and the CLI implements an edit as `popAll` + re-enqueue with no write path from outside.

**A run regin owns reports its own status.** Every other session's phase is inferred from span timestamps (`_session_phase`, a short working window and a staleness threshold), and for this tier that inference is actively wrong: a five-minute tool call emits no spans, so a child plainly at work ages past the threshold and the card reads "inactive". `registry.run_phase` answers from the runner in the same process instead — `starting` (id reserved, child not up), `waiting-input`/`waiting-permission` read straight off `_asks` rather than guessed from whether a PENDING span happens to be newest, `stopping`, `working` (a turn open, including one the agent opened for itself after a background task woke it), else `idle`. `_session_summary` passes it as `main_override`, so it replaces main's span-derived phase while subagents keep their roster-derived ones. `None` means "not ours" and the heuristic stands, which is all a terminal session has.
...
**Cancel is session-scoped for the same reason.** `POST /api/sessions/<id>/bridge-interrupt` is a third route on that pattern rather than a second control on `/api/agent-runs`: both tiers can cancel a turn and the operator pressing it is not thinking about which one they are on, so the route picks the transport — a typed interrupt for a run regin owns, and for one it merely traces the `Escape` a human would press in that pane (`deliver_key`, which spends a `rate_limit_per_minute` token like any keystroke). Stop stays on `/api/agent-runs` because only this tier can be ended from a browser at all.

**A launched run is traced twice, and both ids steer it.** The run has regin's own id (`sdk-…`), while the child — loading the user's hooks like any other session — reports a `claude` session of its own, so `/live` can be opened on either. `registry.register_alias` records the child's id (learned from the first SDK message carrying one) against the run, and every registry entry point normalizes through `owning_run`, so `is_sdk_owned`, the composer, the answer sheet and the decision card resolve the same channel whichever id the operator is looking at. Relatedly, the child inherits this process's environment: `client.py` forces `REGIN_BRIDGE=0` on it, or its own SessionStart hook would register the pane regin's *server* runs in as its bridge pane, and a steer would be typed into the operator's terminal.
...
**A parked call rings the phone.** Holding a call is only a feature if someone is told: the hook tier pushes `permission.pending` through `lib/agent_messages/event_notify`, so before the runner did the same a session regin *owned* was the silent one. Since a park waits indefinitely, this push is the only thing standing between "held for you" and "hung". The push runs off the shared loop (the channels do network I/O) and is best-effort in both directions: a dead webhook must not break a park, and a park must not leave a stale card. The card is keyed per *session* while parks are keyed per *call*, so the dismissal waits until the session has nothing else parked — otherwise resolving the first of several calls would retire a notice that is still true.

**Resuming a session the user drove opens a new trace; resuming regin's own run continues its trace.** `resume` reaches `ClaudeAgentOptions` either way, and `session.start` stamps `resumed_from` with the CLI session being continued (kept through the `/map` attr whitelist). For a session the user drove in a terminal, that id is the trace id and the continuation gets an `sdk-…` trace of its own — writing into a trace regin does not own would be asserting a link it cannot verify. For a run regin launched, `resume` names the `agent_runs` row rather than a CLI session: the route (`_resume_target` in `web/blueprints/agent_runs.py`) reads that row's `cli_session_id`, hands *that* to the CLI, and relaunches under the run's existing `sdk-…` id, so `upsert_run` revives the row and both halves keep landing on one trace. The row is found by *either* id (`store.find_run`) because the id an operator arrives with is normally the child's — that is what the session list shows and `/live` routes on — and resolving only the `sdk-…` one would take the pass-through path and mint a second run against the same child, which `trace_group` then hides. Both resolvers tiebreak an ambiguous child with `ORDER BY trace_id LIMIT 1`, so resume and the trace view always pick the same row. Reviving clears `detail`: it explains how a run *ended*, and a row back at `running` still carrying "stopped" contradicts itself to anything polling it. `fork_session=False` is pinned in `client.build_options` to make this hold — a fork would give the child a new session id and strand the first half's trace under the old one, and `store.set_cli_session` refuses (rather than overwrites) a second, different id for the same reason. A run is offered as resumable only once its child has reported (`cli_session_id IS NOT NULL`) and only while nothing owns it, which is what `resumable` on `GET /api/agent-runs/<id>` reports and the `/live` launch sheet renders.

**A run owns its trace id from the moment it is scheduled, not from the moment its runner exists.** `register_run` fires only after `connect()` has spawned the child, so "nothing owns it" was false for the whole of a run's startup: two resumes landing in that window were both admitted, the second runner replaced the first in `_runs`, and the first's teardown then evicted *it* — a live child `/live` could neither stop nor steer, holding its `max_concurrent_runs` slot until the server restarted. `supervisor.launch_run` therefore claims the id before scheduling anything (`registry.reserve_run` — the test and the claim under one `_lock` acquisition, so of two racing callers exactly one proceeds), and the loser is a `LaunchRefused`, which the launch route already reports as a structured refusal rather than a 500. A reservation counts as owned (`is_sdk_owned`) and against capacity (`active_run_count`, which the runner's own check discounts itself from), but deliberately puts nothing in `_runs`: `stop_run` and `submit_prompt` call methods on whatever they find there, and a run that is still connecting genuinely is *not reachable yet*. The claim is given back on every terminal path — returned, raised, cancelled, and a scheduling failure that never produced a future — because an id held forever could never be resumed again, which is worse than the race it closes; releasing names the token that claimed it, so a run ending late cannot take back an id a successor now holds.
...
**The first caller is topic drafting** (`lib/topics/proposal_external.py`, gated by `topic_evolution.proposal_sdk_runner` — on by default — plus `agent_sdk.enabled`, which is not). The completion contract is untouched — `proposal-finish` is still authoritative, the temp-file handshake is still the fallback, `REGIN_LLM_SURFACE` still tags the agent's own session `origin='llm-stage'` — but the run is owned by the shared loop instead of a `Popen` on one worker thread's stack, so it survives the request, is watchable and stoppable on `/live`, and names its own `agent_trace_id` instead of the subprocess path's post-hoc search for the session the spawned CLI happened to open. Stop still routes through `run_control`, which stores a `Popen`-shaped adapter (`_SdkRunProcess`) for these runs. The gate also checks the *configured* agent's command: the tier launches the user's `claude`, so a run bound to codex/kimi keeps its own binary and its own subprocess.

The tier as a whole is gated off by default (`settings.agent_sdk.enabled = False` — the single switch an operator turns on; `proposal_sdk_runner` above only decides whether drafting rides it once it is), and the SDK is an optional extra (`pip install -e ".[agent-sdk]"`) since observing sessions needs nothing from it.
