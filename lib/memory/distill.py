...
# The bar an entry must clear is deliberately spelled out, and "return
# []" is legitimized: an open-ended "list up to N lessons" anchors the
# model to produce exactly N, padding with generic process advice that
# pollutes recall. Selectivity over coverage.
_DISTILL_PROMPT = """<role>
...
def _compose_prompt(trace_id: str, spans: list[dict], grade: "dict | None",
                    python: str) -> str:
    """Assemble the agentic distill prompt: investigation instructions
    (the agent self-fetches the trace), then the high-signal hints — the
    grader's findings and the heuristic notable-signals — and the output
    contract. The raw trace is NOT embedded; the agent reads it on demand,
    so prompt size never scales with session length."""
    head = _DISTILL_PROMPT.replace("{trace_id}", trace_id).replace(
        "{python}", python)
    sections = [head.rstrip(),
                _tagged("grader_findings", _grade_digest(grade)),
                _tagged("notable_signals", _signal_digest(spans)),
                _OUTPUT_FORMAT]
