...
def test_hard_ignore_records_negative_exemplar(monkeypatch):
    """A hard ignore on a recorded query writes a `MemoryNegative` (the
    contextual demotion signal), gated on the demotion being enabled."""
    from sqlmodel import select
    from lib.memory.engine import MemorySessionLocal
    from lib.memory.models import InjectionEvent, MemoryNegative
    from lib.memory.store import SqliteMemoryStore
    from tests.memory.test_store import _StubEmbedder

    monkeypatch.setattr(settings.agent_memory, "negative_demotion_weight", 0.5)
    store = SqliteMemoryStore(embedder=_StubEmbedder({}))
    mid = _remember(store, title="unrelated",
                    body="never touch `lib/never/referenced.py` casually")
    with MemorySessionLocal() as session:
        session.add(InjectionEvent(
            session_id="sess-neg", memory_id=mid,
            injected_at="2026-06-11T10:00:00",
            query="how do I fix the parser bug"))
        session.commit()
    _insert_span("sess-neg", "tool.Edit", {"file_path": "lib/other/thing.py"},
                 "2026-06-11T10:05:00", "sp-neg")

    result = score_injection_usefulness("sess-neg", store)

    assert result.ignored == 1
    with MemorySessionLocal() as session:
        rows = session.exec(select(MemoryNegative)
                            .where(MemoryNegative.memory_id == mid)).all()
    assert len(rows) == 1
    assert rows[0].source_session == "sess-neg"


def test_hard_ignore_records_no_negative_when_disabled(monkeypatch):
    """With the demotion off (default), feedback records no negatives — the
    ...
    with MemorySessionLocal() as session:
        session.add(InjectionEvent(
            session_id="sess-off", memory_id=mid,
            injected_at="2026-06-11T10:00:00", query="some prompt text"))
        session.commit()
    _insert_span("sess-off", "tool.Edit", {"file_path": "lib/other/thing.py"},
                 "2026-06-11T10:05:00", "sp-off")

    score_injection_usefulness("sess-off", store)

    with MemorySessionLocal() as session:
        assert session.exec(select(MemoryNegative)).all() == []


def test_engaged_records_positive_exemplar(monkeypatch):
    """An engaged inject on a recorded query writes a positive `MemoryExemplar`
    (the contextual boost signal), gated on the boost being enabled."""
    from sqlmodel import select
    from lib.memory.engine import MemorySessionLocal
    from lib.memory.models import InjectionEvent, MemoryExemplar
    from lib.memory.store import SqliteMemoryStore
    from tests.memory.test_store import _StubEmbedder

    monkeypatch.setattr(settings.agent_memory, "positive_boost_weight", 0.5)
    store = SqliteMemoryStore(embedder=_StubEmbedder({}))
    mid = _remember(store, title="Read first",
                    body="Edit fails on `lib/memory/store.py` if not Read first.")
    with MemorySessionLocal() as session:
        session.add(InjectionEvent(
            session_id="sess-pos", memory_id=mid,
            injected_at="2026-06-11T10:00:00",
            query="how do I edit the store"))
        session.commit()
    _insert_span("sess-pos", "tool.Edit", {"file_path": "lib/memory/store.py"},
                 "2026-06-11T10:05:00", "sp-pos")

    result = score_injection_usefulness("sess-pos", store)

    assert result.engaged == 1
    with MemorySessionLocal() as session:
        rows = session.exec(select(MemoryExemplar)
                            .where(MemoryExemplar.memory_id == mid,
                                   MemoryExemplar.polarity == 1)).all()
    assert len(rows) == 1
    assert rows[0].source == "auto" and rows[0].source_session == "sess-pos"


def test_engaged_records_no_positive_when_disabled(monkeypatch):
    """With the boost off (default), feedback captures no positives — the
    feature stays fully opt-in, like the demotion."""
    from sqlmodel import select
    from lib.memory.engine import MemorySessionLocal
    from lib.memory.models import InjectionEvent, MemoryExemplar
    from lib.memory.store import SqliteMemoryStore
    ...
    with MemorySessionLocal() as session:
        session.add(InjectionEvent(
            session_id="sess-pos-off", memory_id=mid,
            injected_at="2026-06-11T10:00:00", query="edit the store"))
        session.commit()
    _insert_span("sess-pos-off", "tool.Edit", {"file_path": "lib/memory/store.py"},
                 "2026-06-11T10:05:00", "sp-pos-off")

    score_injection_usefulness("sess-pos-off", store)

    with MemorySessionLocal() as session:
        assert session.exec(select(MemoryExemplar).where(
            MemoryExemplar.polarity == 1)).all() == []
