...
// Bug C1's regression test was removed: it asserted that
// `useQueuedPrompts`' client-side optimistic echo could not leak across a
// session switch, and that echo no longer exists. The composable is now a pure
// render of the server's `queued_prompts` for the current session id — "no
// client-side optimistic echo and no TTL" — so the leak it guarded is not
// reachable. Restore it only alongside a client-held queue.
