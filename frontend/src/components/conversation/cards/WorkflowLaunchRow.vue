...
<template>
  <div
    tabindex="0"
    data-testid="workflow-launch-row"
    class="flex flex-wrap items-center gap-x-2 gap-y-1 cursor-pointer rounded-lg border border-transparent px-2.5 py-1 hover:bg-emerald-50/60 focus-visible:outline-2 focus-visible:outline-emerald-500"
    :class="selectedSpan && selectedSpan.span_id === span.span_id ? 'event-selected' : ''"
    @click="$emit('activate', span)"
  >
    <span class="text-[12.5px] font-semibold text-emerald-700 shrink-0">Workflow</span>
    <span
      v-if="span.attributes?.workflow_name"
      class="font-mono text-[12.5px] text-slate-500 truncate flex-1 min-w-0"
