            ...
            <tr v-for="r in recRows" :key="r.session_id + r.topic_id" class="border-t border-slate-100 hover:bg-slate-50/60">
              ...
              <td class="px-3 py-2">
                <div class="flex items-center justify-end gap-0.5">
                  <Button variant="ghost" size="sm" :class="['p-2 min-h-9 min-w-9 h-auto focus-visible:ring-2', thumbAttrs(r, 'positive').cls]" :disabled="!r.query || busy === rowKey(r)" :aria-pressed="thumbAttrs(r, 'positive').pressed" :aria-label="thumbAttrs(r, 'positive').title" :title="thumbAttrs(r, 'positive').title" @click="judge(r, 'positive')"><Icon name="thumbs-up" :size="14" /></Button>
                  <Button variant="ghost" size="sm" :class="['p-2 min-h-9 min-w-9 h-auto focus-visible:ring-2', thumbAttrs(r, 'negative').cls]" :disabled="!r.query || busy === rowKey(r)" :aria-pressed="thumbAttrs(r, 'negative').pressed" :aria-label="thumbAttrs(r, 'negative').title" :title="thumbAttrs(r, 'negative').title" @click="judge(r, 'negative')"><Icon name="thumbs-down" :size="14" /></Button>
