...
const STAGES = [
  { n: 1, name: 'pre-pass', kind: 'auto', sub: 'dedup merge · engagement scoring · legacy-digest retire',
    counts: (r) => [['merged', r.merged]] },
  { n: 2, name: 'dream', kind: 'llm', sub: 'one plan per run: row fates · pair verdicts · syntheses → review queue',
    surface: 'memory-reflect-dream',
    counts: (r) => [
      ['promoted', r.promoted], ['held', r.held], ['dropped', r.dropped],
      ['contradicted', r.contradictions], ['obsoleted', r.obsoleted],
      ['synthesized', r.synthesized], ['pairs', r.pairs_checked],
      ['skipped', r.dream_skipped],
    ],
    control: { type: 'check', key: 'dream_enabled', label: 'enabled' },
    extra: { type: 'check', key: 'promote_allow_retire', label: 'may retire' } },
  { n: 3, name: 'forget · decay · flag-stale', kind: 'auto', sub: 'age + recall signals',
    counts: (r) => [['decayed', r.decayed], ['forgotten', r.forgotten], ['flagged', r.flagged_stale]] },
  { n: 4, name: 'embed · edges', kind: 'auto', sub: 'dense model · similarity links',
