...
<template>
  ...
  >
    ...
    <div
      ref="bodyEl"
      class="relative text-sm text-slate-800 leading-relaxed"
      :class="expanded ? '' : 'max-h-72 overflow-hidden'"
    >
    ...
    <div v-if="overflowing" class="mt-1 flex items-center gap-3">
      <Button
        variant="link"
        class="text-[11px] font-medium pointer-coarse:min-h-9"
        @click="expanded = !expanded"
      >{{ expanded ? 'Show less' : 'Show more' }}</Button>
      <router-link
        v-if="!expanded && sessionHref"
        :to="sessionHref"
        class="inline-flex items-center text-[11px] font-medium text-blue-600 hover:text-blue-800 no-underline rounded focus-visible:outline-2 focus-visible:outline-blue-500 pointer-coarse:min-h-9"
    ...
    <ul v-if="classifiedLinks.length" class="mt-2 flex flex-wrap gap-1.5">
      <li v-for="(lnk, i) in classifiedLinks" :key="i" class="min-w-0 max-w-full">
        <a
          ...
          target="_blank"
          rel="noopener"
          class="inline-flex items-center gap-1 min-w-0 max-w-full text-[11px] font-mono text-blue-600 hover:text-blue-800 bg-blue-50 px-1.5 py-0.5 rounded no-underline focus-visible:outline-2 focus-visible:outline-blue-500 pointer-coarse:min-h-9"
        ...
        <Button
          ...
          class="h-auto min-h-0 justify-start gap-1 min-w-0 max-w-full text-[11px] font-mono font-normal text-slate-600 hover:text-slate-800 bg-slate-100 hover:bg-slate-200 px-1.5 py-0.5 rounded pointer-coarse:min-h-9"
    ...
    <div class="mt-2.5 flex items-center gap-3 text-[11px] text-slate-500">
      <router-link
        v-if="sessionHref"
        :to="sessionHref"
        class="inline-flex items-center gap-1 text-slate-500 hover:text-blue-600 no-underline truncate max-w-[60%] rounded focus-visible:outline-2 focus-visible:outline-blue-500 pointer-coarse:min-h-9"
        @click="emit('open', message)"
      >
