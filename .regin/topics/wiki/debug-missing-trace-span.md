...
A Claude Code / Codex / generic-provider session shows fewer rows in regin's `session_spans` table than the on-disk JSONL transcript holds. This topic walks an agent end-to-end through diagnosis and recovery using only shell + the regin venv. The whole surface is two modules: the `turn_trace` handler package (`hook_manager/handlers/turn_trace/`) that emits spans on every hook fire, and `lib/trace/repair.py`, whose `repair_session_spans` replays that same emit pipeline to recover what was lost.
...
* Subagent markers (`subagent.start` / `subagent.stop`) or a subagent's `tool.*` spans are absent while its assistant turns render.
...
Two equivalent entry points. **Prefer the direct-Python path** — it surfaces tracebacks and bypasses the auth interceptor (`web/app.py::_install_auth_gate`, line 266).
...
This still requires the regin web server running on `http://127.0.0.1:8321`, because `lib.trace.repair.repair_session_spans` (`lib/trace/repair.py:729`) re-runs the live emit pipeline and that pipeline posts spans via `lib.hook_plugin.post_span` (`lib/hook_plugin.py:494`) to the public ingest endpoint `trace.api_ingest_session_span` (allowlisted in `web/app.py:226` `PUBLIC_API_ENDPOINTS`).

**HTTP via curl** — the endpoint `api_session_repair_spans` (`web/blueprints/trace/sessions.py:2200`) is not in `PUBLIC_API_ENDPOINTS`, so an Authorization header is required:
...
```
{
  ...
  "uuids_unlocked": <N - M>,
  "spans_recovered": <K>,
  "subagent_markers": {"agents_on_disk": <A>, "starts_synthesized": <S>, "stops_synthesized": <T>},
  "tool_backfill": {"spans_backfilled": <B>, "transcripts_walked": <W>}
}
```
...
* `subagent_markers.starts_synthesized > 0` or `tool_backfill.spans_backfilled > 0` → the loss was in a span class with no ordinary replay path (SubagentStart/Stop markers or lost `tool.*` calls), and it was recovered from the on-disk transcripts rather than from the seen-uuid cache.
* `uuids_unlocked == 0` (and all recovery counts 0) with the user still reporting missing turns → this isn't the seen-uuid lockout. Investigate the transcript parser (`lib/trace/transcript_usage.py::read_usage`) or the projection grafter (`lib/trace/projection.py::_graft_orphans`, line 510).
...
The new row counts for `assistant_response`, `assistant.thinking`, `harness.local_command`, `harness.skill_listing`, `harness.task_reminder`, `harness.tools_delta`, the synthetic `srvtool-*` / `askdeny-*` / `tooldeny-*` / `toolerr-*` / `sys-*` / `prompt-*` spans, and any recovered `subagent.start` / `subagent.stop` markers and backfilled `tool.*` rows should account for the recovery counts.
...
The materialize endpoint is `api_session_materialize` (`web/blueprints/trace/sessions.py:2172`); it wraps the fetch + orphan-graft + envelope-widen projection in a `BEGIN IMMEDIATE` transaction and returns `parent_updates` / `envelope_updates` counts.

### Step 8 — Confirm repair telemetry

`repair_session_spans` writes one `span_repair_completed` line via `lib.activity_log.get_activity_logger('trace_ingest')` (`lib/trace/repair.py:811`), tagged with `uuids_dropped`, `spans_recovered`, and `transcript_path`. The marker and tool-backfill sub-steps log their own `subagent_markers_reconstructed` / `tool_spans_backfilled` rows under the same feature:
...
* Each emitter in `hook_manager/handlers/turn_trace/span_posters.py` collects `new_uuids` only after a successful post and then calls `_mark_seen`:
  * `_post_system_event_spans` (line 163, marks at 181).
  * `_post_attachment_spans` (line 342, marks at 376).
  * `_post_local_command_spans` (line 382, marks at 437).
  * `_post_live_turn_data` (line 1224, marks at 1265); the per-turn helper `_maybe_emit_assistant_span` (line 1131) returns the post's bool so its uuid is cached only on success.
...
`lib/trace/repair.py::repair_session_spans` (line 729) is idempotent — re-running on a healed session is a no-op because the cache no longer holds any uuid whose expected spans are missing. The algorithm:

1. `_find_transcript(trace_id)` (`repair.py:174`) scans the active provider's `transcript_projects_dir()` for the matching `<trace_id>.jsonl`. Missing → returns `{ok: false, error: 'no transcript found for ...'}` (HTTP 404).
2. `_load_cache` (`repair.py:224`) reads the seen-uuid file; `_existing_span_ids` (`repair.py:208`) snapshots existing span_ids from `session_spans`.
3. `_expected_span_ids_by_uuid` (`repair.py:134`) walks the transcript via `read_usage` and asks `_turn_expected_span_ids` (line 59), `_attachment_expected_span_ids` (line 89), and `_add_local_command_expected` (line 105) what span_ids each cached uuid should map to. Local-command is subtle: a slash command's caveat uuid and stdout uuid never appear in their own span_id, so all three (command / caveat / stdout) fold into the command-uuid's `cmd-*` expected set. Without this fold, repair would unlock the caveat/stdout uuids on every run.
4. Walk the cache; `_should_reemit` (`repair.py:246`) drops a uuid whose expected set is not yet a subset of `existing`, keeps the rest.
5. Rewrite the cache **before** re-emitting (`_save_cache`, `repair.py:234`) — a failed rewrite would risk double-caching.
...
7. Recover the span classes that have **no ordinary replay path**. `reconstruct_subagent_markers` (`repair.py:382`) synthesizes `subagent.start` / `subagent.stop` markers (deterministic `substart-sa-<agent_id>`) from the on-disk `subagents/agent-*.jsonl` transcripts — SubagentStart/Stop hooks fire exactly once, so an ingest outage loses them permanently and leaves ghost agents whose `agent_id`-tagged spans have nothing to hang off. `backfill_transcript_tool_spans` (`repair.py:695`) re-emits `tool.*` spans (deterministic `bftool-<tu>`) the live PostToolUse path lost, walking the main transcript and every subagent transcript and skipping already-resolved `tool_use_id`s. Both are idempotent.
8. Call `materialize_session` (best-effort) so the healed parents + envelopes are durably recorded, then diff the span_id set before vs. after and return `{ok, trace_id, transcript_path, cached_uuids_before, cached_uuids_after, uuids_unlocked, spans_recovered, subagent_markers, tool_backfill}`, logging `span_repair_completed` (`repair.py:811`) via `lib.activity_log` (feature `trace_ingest`).

Replay safety relies on the span ingest path using `INSERT INTO session_spans ... ON CONFLICT(trace_id, span_id) DO UPDATE SET ...` (`lib/trace/trace_service/ingest.py:1143`). Deterministic span_ids (`resp-*`, `think-*`, `srvtool-*`, `askdeny-*`, `tooldeny-*`, `toolerr-*`, `sys-*`, `att-*`, `cmd-*`, `prompt-*`, `skill-init-*`, `sttl-*`, `substart-sa-*`, `bftool-*`) guarantee a replayed transcript only upserts.
...
* Trace capture and the span vocabulary this recovers: [[trace-span-capture]] (builders, payload schemas, `docs/trace/SPAN_DESIGN.md`); the transcript parser the whole pipeline reads from is `lib.trace.transcript_usage.read_usage`.
* The serve-time reconcile that turns raw spans into the rendered tree: [[trace-merge-reconcile]] — `lib/trace/projection.py::_graft_orphans` (line 510) and `_graft_orphans_under_prompt` (line 248) anchor orphan `assistant_response` spans under the nearest prior `prompt` span (recovered responses may show as orphans until the corresponding prompt uuid is also unlocked), plus `_widen_envelopes` (line 553).
* The ingest / aggregate / persistence internals behind the endpoints: [[session-trace-design]].
...
* The auth interceptor that gates the HTTP repair endpoint: `web/app.py::_install_auth_gate` (line 266), with the `PUBLIC_API_ENDPOINTS` allowlist (line 226).
* The ingest-error observability surfaces: file `<provider.traces_dir()>/ingest-errors.jsonl` + HTTP `/api/ingest-errors` (`web/blueprints/trace/ingest_errors.py:42`).
